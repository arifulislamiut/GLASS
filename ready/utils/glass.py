...
class GLASS(torch.nn.Module):
    ...
    def _evaluate(self, images, scores, segmentations, labels_gt, masks_gt, name, path='training'):
        ...
        if len(masks_gt) > 0:
            ...
            # Check if we have valid pixel-level annotations (not all zeros)
            masks_array = np.array(masks_gt)
            if np.sum(masks_array) > 0:  # If there are any non-zero pixels
                try:
                    ...
                    print(f"Warning: Pixel-wise evaluation failed for {name}: {e}")
                    pixel_auroc = -1.
                    pixel_ap = -1.
                    pixel_pro = -1.
            else:
                # No pixel-level annotations available
                pixel_auroc = -1.
                pixel_ap = -1.
                pixel_pro = -1.
        else:
